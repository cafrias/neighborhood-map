...
class Map {
	...
	populateInfoWindow(marker) {
		if(this.infoWindow.marker != marker) {
			...
			// this.setFoursquareInfoOnContent(marker);
	...
	getFoursquareData(venueId) {
		...
		return new Promise(function(resolve, reject) {
			...
			clientId='NYCWRKZ0J5DU51MHNVCONP03345RUKBCP1MFSYBNU3UYLR55';
			clientSecret='1UXWASOVEN2IXK3CTNI3IYZ5DFZQNDZBWFOPN525MMEZ34OX';

			oReq.addEventListener('load', (e) => {
				var {rating} = JSON.parse(e.target.responseText).response.venue;
				resolve({rating});
			});
			...
			oReq.open('GET', `https://api.foursquare.com/v2/venues/${venueId}?client_id=${clientId}&client_secret=${clientSecret}&v=20170401`);

...
class ViewModel {
	...
	constructor(ko, locations=[]) {
		...
		/**
		 * Hold index of currently selected Marker on the Map, defaults to 0.
		 * @type {Number}
		 */
	...
	createMarkersForLocs(locations=[]) {
		locations.forEach((location, idx) => {
			...
			// the listener.
			var handler = this.markerOnClickHandler.bind(this);
	...
	/**
	 * Routine for when marker is clicked, that may happen if either:
	 * - Marker is directly clicked on the map
	 * - Marker is selected from the list in the Sidebar
	 * @param  {Marker} clickedMarker The clicked marker
	 */
	markerOnClickHandler(clickedMarker) {
		// First stop animation on currently selected marker.
		this.stopAnimationOnCurrentMarker();
		// Save index of selected marker.
		this.selectedMarker = this.markers.indexOf(clickedMarker);

		this.animateClickedMarker(clickedMarker);
		this.map.populateInfoWindow(clickedMarker);
	}
	...
	/**
	 * Stop animation on currently active marker represented by `this.selectedMarker`.
	 */
	stopAnimationOnCurrentMarker() {
		var marker = this.markers[this.selectedMarker];
	...
	filterClickAction(data) {	
		...
		this.markerOnClickHandler(data);
